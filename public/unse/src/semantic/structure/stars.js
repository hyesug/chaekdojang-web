...
export function natureOf(stars, who) {
  ...
  // **장점의 뒷면이 곧 걸림돌이다.** 강점만 적으면 "무엇을 조심하나"에 답할 수가
  // 없어서, 별마다 같은 성질이 어떻게 발목을 잡는지 함께 적는다
  const risk = [...new Set(main.map((s) => STAR_NATURE[s].risk).filter(Boolean))];

  const L = [];
  L.push(`${who}의 결은 ${j(main.length ? main.join('·') : help.join('·'), '이')} 그립니다.`);
  if (nature.length) L.push(nature.join(' / ') + '.');
  L.push(`구체적으로는 **${traits.join(' · ')}**.`);
  if (toward.length) L.push(`자라는 방향은 ${toward.join(' / ')}입니다.`);
  if (risk.length) L.push(`같은 성질이 걸림돌이 되는 자리는 **${risk.join(' / ')}**.`);
  if (main.length >= 2) {
    L.push(`별이 ${main.length}개 들어 결이 한 갈래가 아닙니다 — 상황에 따라 다른 면이 나옵니다.`);
  }
  return { stars: [...main, ...help], nature, traits, toward, risk, text: L.join(' ') };
}
