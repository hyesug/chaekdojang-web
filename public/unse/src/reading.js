...
const GOD_LIFE = {
  ...
  정인: '배움과 도움이 붙는 때입니다. 익힌 것을 정리하거나 도움을 받아들이기에 좋은 때입니다',
};
...
/** 월주 — 한창때. 직업과 사회생활의 결 */
const GOD_AT_MONTH = {
  비견: '남이 정한 방식만 따르기보다 스스로 기준과 책임을 잡는 방식에서 힘이 붙습니다',
  겁재: '사람과 같이 벌이는 일이 많은데, 돈 계산만 분명히 해두면 크게 갑니다',
  식신: '만들고 기르는 방식으로 자리를 잡습니다. 한 분야를 오래 붙드는 쪽이 유리합니다',
  ...
  정인: '배운 것이 그대로 값이 되는 결입니다. 쌓은 것을 남에게 전하는 자리에서 힘이 붙습니다',
};
...
const GOD_AT_HOUR = {
  ...
  편인: '말년에는 안으로 파고드는 시간이 늘어날 수 있습니다. 바깥 자극이 줄면 생각이 깊어지는 대신 고립감이 커질 수 있으니 생활 리듬을 일정하게 두는 편이 낫습니다',
 ...
 */
const WORK_STYLE = {
  비겁: '내 몫과 재량이 분명한 구조에서 힘이 붙습니다. 누가 무엇을 했는지 드러나는 방식일수록 오래 가고, 공이 뭉뚱그려지는 자리에서는 금세 지칩니다.',
  식상: '만들어서 보여주는 방식이 맞습니다. 결과물이 손에 잡히고 절차보다 산출물로 평가받는 구조에서 빨리 크며, 과정을 말로만 설명해야 인정받는 자리에서는 답답해합니다.',
  재성: '투입과 결과가 바로 연결되는 구조가 맞습니다. 들인 것이 숫자로 확인될수록 동기가 붙고, 한 건을 오래 붙드는 것보다 여러 갈래를 동시에 굴리는 쪽에서 성과가 납니다.',
  관성: '기준이 있는 구조 안에서 복잡한 것을 정리하고 개선하는 데 강합니다. 흩어진 정보를 묶어 기준을 세우고 결과로 만드는 역할에서 강점이 드러나기 쉽습니다. 다만 오래 통제만 받는 자리보다는, 쌓인 전문성만큼 재량이 함께 커지는 방향이 유리합니다.',
  인성: '배우고 정리해서 전하는 방식이 맞습니다. 쌓은 것이 시간이 갈수록 값이 되는 구조에서 오래 가고, 매번 처음부터 다시 시작해야 하는 자리에서는 힘이 빠집니다. 다만 준비가 길어지기 쉬우니 실전에 들어갈 시점을 미리 못 박아두는 편이 낫습니다.',
};
...
export function lifeReading(input, chart, synth) {
  ...
  return {
    ...
    body:
      ...
      `여기 적은 것은 기질에서 읽은 생활 리듬이지 몸 상태를 판단한 것이 아닙니다. ` +
      `불편한 데가 있으면 전문가에게 보이셔야 합니다.`,
...
const DAY_LINE = {
  ...
  나쁨: [
    ...
    '오늘 내보낸 것은 되돌리기 어렵습니다. 되돌릴 수 없는 결정은 하루 미루세요.',
...
const YEAR_KIND = {
  ...
  겁재: { tag: '지출', body: '기회와 나가는 자리가 같이 들어오는 해. 함께 벌이는 일에서 내 몫의 경계가 흐려지기 쉬운 때' },
  ...
  정관: { tag: '자리', body: '맡은 자리와 역할이 또렷해지는 해. 남이 인정하는 형태로 결과가 정리되는 일이 주제로 올라오는 때' },
  편인: { tag: '공부', body: '남들과 다른 공부와 정보에 빠지는 해. 배움에는 좋으나 결정이 늦어지는 때' },
  정인: { tag: '문서', body: '배움과 도움이 붙는 해. 형식을 갖춰 남기는 일이 잘 풀리는 때' },
};
...
const YEAR_HIT = {
  ...
  year: {
    충: '집안이나 윗대 쪽이 움직이는 때. 뿌리 쪽 사정으로 일정과 마음이 끌려가기 쉬운 때',
...
const P = {
  ...
  금전운: {
    ...
    mid_lo: ['들어오는 것과 나가는 것이 자주 엇갈립니다. 손에 쥐기 전에 나갈 자리가 생기니 실제로 남는 것은 적습니다. 이런 때는 늘리는 쪽보다 나가는 쪽을 먼저 보셔야 하고, 평소보다 큰 것을 기대하는 결정은 흐름을 거스릅니다.'],
    lo: ['새는 자리가 생깁니다. 남의 몫까지 떠안게 되는 결정은 특히 조심하셔야 하고, 큰 것은 다음 시기로 넘기는 편이 낫습니다. 이 시기에 내보낸 것은 되돌리기 어렵습니다.'],
  },
  애정운: {
  ...
  학업운: {
    hi: ['머리가 맑습니다. 어려운 것부터 손대고, 오래 걸리는 공부를 이 시기에 몰아두면 효율이 좋습니다.'],
  ...
  건강운: {
    hi: ['몸이 가볍습니다. 미뤄둔 일 가운데 체력이 드는 것을 이 시기로 당기면 수월합니다.'],
    mid_hi: ['회복이 잘 됩니다. 무리만 하지 않으면 탈이 없습니다.'],
    mid: ['큰 이상 없이 지나갑니다. 평소 리듬만 지키세요.'],
    mid_lo: ['생활 리듬이 쉽게 흐트러지는 때입니다. 일정과 활동량을 평소보다 줄이고, 회복 시간을 먼저 확보하는 편이 낫습니다.'],
    lo: ['소모가 큽니다. 쉬는 것을 일정에 넣어두지 않으면 회복이 계속 뒤로 밀립니다.'],
  },
};
...
const GOD_AREA = {
  비겁: {
    ...
    금전운: '나가는 자리가 사람 쪽에서 생깁니다. 관계를 이유로 쓰게 되는 몫이 겹치기 쉬우니, 한도를 먼저 정해두는 편이 낫습니다.',
  ...
  식상: {
    ...
    금전운: '결과가 분명하게 드러날수록 보상과 연결되기 쉬운 흐름입니다. 규모를 크게 잡기보다 실제로 남는 것을 확인해가며 넓히는 편이 낫습니다.',
  ...
  관성: {
    ...
    금전운: '나가는 쪽이 이미 정해져 있는 결입니다. 미룰 수 없는 것부터 확인해두면 나머지가 훨씬 수월해집니다.',
    직장운: '위에서 보는 눈이 늘어납니다. 맡은 것을 기한 안에 끝내는 것만으로 평가가 올라갑니다.',
    학업운: '기준이 분명한 공부에 맞습니다. 스스로 정한 규칙을 지키는 일이 그대로 결과가 됩니다.',
    건강운: '긴장이 생활 리듬까지 번지기 쉽습니다. 회복 시간을 미리 비워두고, 무리한 일정이 이어지지 않게 조절하는 편이 낫습니다.',
  },
  인성: {
    총운: '도움과 배움 쪽으로 길이 열립니다. 혼자 버티기보다 물어보고 기대는 편이 빠릅니다.',
    애정운: '보살피고 보살핌받는 관계가 편해집니다. 다만 상대를 가르치려 들면 그 자리에서 어긋납니다.',
    금전운: '크게 불리는 결이 아니라 새는 것을 막는 결입니다. 정해둔 조건을 한 번 더 확인하는 것만으로 지켜지는 몫이 있습니다.',
    직장운: '형식을 갖춰 남기는 일이 유리합니다. 말로 오간 것은 기록으로 옮겨두는 편이 낫습니다.',
...
const PAIR_TEXT = {
  ...
  오래: {
    ...
    mid: '오래 갈 여지는 있습니다. 다만 저절로 되는 것은 아니고 맞춰야 할 지점도 뚜렷해서, 고비마다 한 번씩 매듭을 지어야 이어집니다.',
...
const TAG_MEANING = {
  ...
  분석: ['근거가 서야 움직이는 쪽', '조건과 전제를 먼저 확인하고 나서 정하기 때문에 큰 실수가 적습니다'],
  ...
  재물: ['자원이 어디로 흐르는지에 감각이 있는 쪽', '무엇이 값이 되고 무엇이 비용인지를 남보다 빨리 구분합니다'],
  명예: ['자기 역할이 어떻게 불리는지를 중요하게 여기는 쪽', '자기 기여가 분명히 드러나는 환경을 중요하게 느끼고, 몫이 뭉뚱그려지면 의욕이 크게 떨어집니다'],
};
...
const NO_ELEMENT = [
  ...
  '불의 기운이 사주에 없습니다. 드러내고 표현하는 힘이 약해 속으로만 삭이기 쉽고, 기운이 처질 때 혼자 끌어올리기가 어렵습니다. 남에게 드러내야 하는 자리를 일부러 만들어두는 편이 낫습니다.',
...
const GOD_NONE = {
  ...
  재성: '자원을 쌓아 두는 자리가 옅습니다. 적게 번다는 뜻이 아닙니다. 다만 들어오는 것과 나가는 것의 선을 미리 정해두지 않으면, 손에 쥐는 단계까지 가기 전에 흩어지기 쉬운 구성입니다.',
  관성: '자리와 책임의 자리가 겉으로 드러나 있지 않습니다. 남이 정해준 틀보다 스스로 만든 기준에서 힘이 붙는 쪽인데, 그만큼 기준을 직접 세워야 하는 부담도 같이 옵니다.',
  인성: '기대고 배우는 자리가 겉으로 드러나 있지 않습니다. 형식적인 인정 하나에만 의존하면 답답함을 크게 느낄 수 있으므로, 실제로 해낸 과정과 결과를 함께 남겨두는 편이 유리합니다. 어떤 성과도 이 구성 하나만으로 결정되지는 않습니다.',
};
  ...
  const head =
    level === 'strong'
      ...
      : level === 'some'
        ...
        : '어느 한쪽으로 크게 쏠리지 않고 고르게 퍼진 구성입니다. 큰 기복 없이 흘러가는 대신, 무엇 하나가 특별히 앞서지도 않아 방향은 스스로 정해야 합니다.';
...
const TABOO_GOD = {
  비겁: { head: '가까운 사이에서 셈의 경계를 흐리는 것', text: '사람과 자원이 얽힐 때 경계가 흐려지기 쉬운 구성입니다. 가까울수록 누가 무엇을 얼마나 지는지를 미리 말로 정해두는 편이 낫고, 그 선이 없으면 나중에 셈보다 관계 쪽에서 더 크게 잃습니다' },
  ...
  인성: { head: '충분히 준비될 때까지 시작을 미루는 것', text: '준비가 더 필요하다고 느껴 시작 시점을 계속 뒤로 미루기 쉬운 구성입니다. 완벽한 준비를 기다리기보다 일정 수준이 갖춰지면 작은 범위에서 먼저 해보고 보완하는 편이 낫습니다' },
};

/** 비어 있는 자리도 금기를 만든다 */
const TABOO_EMPTY = {
  인성: { head: '형식적인 증명 하나로만 자신을 판단하는 것', text: '외부 기준 하나만으로 자기 능력을 판단하면 준비와 확인이 길어질 수 있습니다. 형식적인 인정과 함께 실제로 해낸 과정과 결과도 같은 무게로 보는 편이 안전합니다' },
  재성: { head: '내가 설명하지 못하는 것에 큰 결정을 맡기는 것', text: '현실 감각을 스스로 믿지 못해 판단을 남에게 넘기기 쉬운 구성입니다. 남이 권했다는 이유만으로 정하면 결과와 상관없이 후회가 남으니, 스스로 설명할 수 있을 때까지는 규모를 키우지 않는 편이 낫습니다' },
...
export function tabooReading(input, chart) {
  ...
  // 일지에 걸린 부딪침을 전부 세면 마흔 명 중 스물여덟에게 같은 문장이
  // 나간다. 그쯤 되면 구성이 아니라 기본값이다. 충과 형만 센다.
  const HEAVY = new Set(['충', '삼형', '상형', '자형']);
  const dayHit = ['year', 'month', 'hour']
    .map((k) => chart.pillars[k] && branchRelations(chart.pillars.day.branch, chart.pillars[k].branch)
      .find((rel) => HEAVY.has(rel.kind)))
    .find(Boolean);
  if (dayHit) {
    out.push({
...
const CLASH_AT = [
  ...
  { at: [1, 2], kind: '충',
    text: '자리와 나 사이가 부딪칩니다. 한곳에 오래 머무르기보다 방향을 한 번 크게 트는 배치이고, 주어진 환경을 그대로 잇기보다 자기 기반을 따로 세우려는 경향이 나타나기 쉽습니다.' },
  { at: [2, 3], kind: '충',
...
function combos(g, isMale) {
  ...
  if (비 >= 2 && 재 <= 1) out.push({
    name: '나눌 몫보다 나눌 사람이 많은 자리',
    text: '같은 것을 바라는 쪽은 여럿인데 나눌 몫은 얇은 구성입니다. 함께 벌이는 일에서 내 몫이 뭉뚱그려지기 쉬운 자리라, 들인 것에 비해 남는 것이 적다고 느끼는 장면이 반복됩니다. 잘못해서가 아니라 구조가 그렇게 생겼습니다. 누가 무엇을 얼마나 지는지를 처음에 분명히 해두는 것만으로 많이 달라집니다.',
  });
  if (인 >= 4 && 식 <= 1) out.push({
    name: '들이는 힘이 내놓는 힘을 누르는 자리',
    text: '받아들이고 쌓는 기운이 지나치게 센 구성입니다. 생각은 깊어지는데 밖으로 내놓는 쪽이 눌려서, 준비가 길어지고 그만큼 결과로 이어지는 데 시간이 걸립니다. 완성한 뒤에 내놓으려는 방식보다, 덜 다듬어진 상태로 먼저 내놓고 고쳐가는 방식이 이 구성에는 잘 듣습니다.',
  });
  if (관 >= 3) out.push({
    name: '누르는 힘이 센 자리',
  ...
  if (인 === 0) out.push({
    name: '형식을 갖춰 증명하는 자리가 옅음',
    text: '종이로 증명하는 자리가 겉으로 드러나 있지 않습니다. 형식을 갖춘 증명 하나에만 기대면 결과가 나오기까지 남보다 시간이 더 걸리기 쉬운 구성이라, 만든 것과 해낸 것으로 보여주는 길을 함께 열어두는 편이 안전합니다. 다만 결과 자체는 구성만으로 정해지지 않고 그때의 운과 준비가 같이 만듭니다.',
  });
  if (식 === 0 && 재 >= 1) out.push({
    name: '내놓는 칸이 비어 있는 자리',
    text: '가진 것이 값으로 바뀌는 가운데 칸이 옅은 구성입니다. 다룰 자원은 보이는데 그것이 내 손으로 오는 통로가 좁아, 들인 것에 비해 돌아오는 것이 적다고 느끼기 쉽습니다. 과정만 반복하기보다 결과가 분명하게 남는 방식으로 한 칸씩 바꿔보는 편이 이 구성에는 잘 맞습니다.',
  });
  if (재 >= 3 && 비 <= 1) out.push({
    name: '다룰 것은 큰데 쥔 힘이 얇은 자리',
    text: '감당해야 할 몫에 비해 그것을 지탱할 힘이 얇은 구성입니다. 판이 커질수록 본인이 먼저 소모되기 쉬운 배치라, 규모를 키우기 전에 나눌 사람이나 맡길 자리를 먼저 만들어두는 편이 안전합니다.',
  });
  return out;
}
