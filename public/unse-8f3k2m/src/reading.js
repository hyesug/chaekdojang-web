...
function combos(g, isMale) {
  ...
  if (식 === 0 && 재 >= 1) out.push({
    ...
    text: '가진 것이 값으로 바뀌는 가운데 칸이 옅은 구성입니다. 다룰 자원은 보이는데 그것이 내 손으로 오는 통로가 좁아, 들인 것에 비해 돌아오는 것이 적다고 느끼기 쉽습니다. 시키는 일을 그대로 해내는 방식보다, 자기 이름이 붙는 결과물을 하나라도 만들어 두는 쪽으로 한 칸만 옮겨도 달라집니다.',
  });
  if (재 >= 3 && 비 <= 1) out.push({
