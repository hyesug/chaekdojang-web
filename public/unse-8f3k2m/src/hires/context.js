 ...
 * 이 값은 **순위이지 탐지기가 아니다.**
 *
 * 점유율은 "이 사람의 이 기간 안에서 몇 번째"라는 상대값이라, "여기서 실제로
 * 일이 벌어졌다"는 절대 기준이 없다. 그런데 순위를 주면 읽는 쪽은 반드시
 * 봉우리를 세고, 세면 개수가 된다. 실제로 그렇게 물어 봤다가 3년 창에
 * 이직 봉우리가 열 개 나왔다.
 *
 * 아래 숫자는 서로 다른 명반 다섯에서 19년(228달)씩 실제로 잰 값이다.
 * 사람 탓이 아니라 계산식의 모양 때문이다 — '전환' 신호가 이직 쪽 네 후보에
 * 더해지고 '현 직장 유지' 하나에서만 빠지는데, 합·충·사화·트랜싯은 조용한
 * 달에도 0이 아니라서 "아무 일 없음"이 구조적으로 진다.
 */
const NOT_A_DETECTOR = [
  '※ **이것은 순위이지 탐지기가 아니다.** 점유율은 이 사람의 이 기간 안에서의 상대값이고, ' +
  '"여기서 실제로 일이 벌어졌다"를 가리는 절대 기준이 아니다.',
  '  실측(서로 다른 명반 다섯, 각 228달): 37달마다 이직 봉우리가 7.8~8.8개씩 나온다. ' +
  "3년에 여덟 번 옮기는 사람은 없다. '현 직장 유지'가 이직 쪽을 이기는 달은 228달 중 0~14달(0~6%)뿐이다.",
  '  그러니 **1위는 "다른 달보다 그 사건에 가깝다"는 뜻이지 "그 일이 일어났다"는 뜻이 아니다.** ' +
  '봉우리를 세어 사건의 횟수를 말하지 말 것.',
];
...
export function formatHiRes(j, plan) {
  ...
  // 판정을 묻는 질문도 순위를 보기 전에 먼저 막는다
  if (plan.asksOutcome) {
    out.push('### ⚠ 이 질문은 **되는가 안 되는가**를 묻는다 — 이 엔진은 판정하지 못한다');
    out.push('달 순위는 **그 사람의 여러 시기를 서로 견준 값**이다. 바깥 상대(회사·학교·심사위원)가 ' +
      '예라고 할지를 재는 자가 아니다. 시기의 강약과 판정은 다른 문제다.');
    if (!plan.hasNegativeCandidate) {
      out.push(`더구나 이 분야(${plan.primary ?? '—'})의 후보 목록에는 **'안 된 쪽'이 아예 없다.** ` +
        '그래서 여기 나오는 점수는 아무리 높아도 "된다"를 뜻할 수 없다 — 질 상대가 없기 때문이다.');
    }
    out.push('**합격·불합격, 된다·안 된다를 단정하지 말 것.** 대신 이렇게 답한다 — ' +
      '"되고 안 되고는 이 계산으로 가릴 수 없다. 다만 이 시기가 당신의 다른 시기에 비해 ~ 하다."');
    out.push('실제로 이것 때문에 틀린 적이 있다. \'합격·수료 상위 17%\'를 근거로 서류합격을 단언했고 결과는 불합격이었다.');
    out.push('');
  }

  // 개수를 묻는 질문은 아래 순위를 보기 전에 먼저 막는다. 뒤에 적으면
  // 이미 봉우리를 센 다음이다
  if (plan.asksCount) {
    out.push('### ⚠ 이 질문은 **횟수**를 묻는다 — 이 엔진은 횟수를 세지 못한다');
    out.push('아래 값은 전부 "이 사람의 이 기간 안에서 몇 번째 달인가"라는 상대 순위다. ' +
      '"여기서 일이 벌어졌다"를 가리는 절대 기준이 없어서, 봉우리를 세면 실제 횟수의 몇 배가 나온다.');
