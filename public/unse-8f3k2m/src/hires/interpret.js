 ...
 *
 * ── **열다섯을 다 세워 봤다** (scripts/validate-axes.mjs) ───────
 * 위 성적은 오래도록 **핵심 넷만** 잰 것이었다. 담당을 정한다면서 후보를
 * 넷만 놓고 골랐던 셈이다. 그래서 나머지 열하나에도 같은 '자기 판/조직'
 * 표를 그 전통의 뜻으로 붙여 열다섯을 한 줄에 세웠다.
 *
 *   수입형태(영점 55%)   1위 자미 4/4 · 육임 5/7 71% · 태을 7/11 64%
 *   직업전환(영점 50%)   1위 토정 8/10 80% · 주역 7/10 · 태을 7/10
 *
 * 표만 보면 담당이 바뀐다. **그런데 순열 검정을 붙이면 사라진다.**
 *
 *   수입형태  열다섯 중 최고 71%  ← 라벨을 섞어도 68.8% 가 그만큼 나온다  p=0.688
 *   직업전환  열다섯 중 최고 80%  ← 라벨을 섞어도 42.3% 가 그만큼 나온다  p=0.423
 *
 * 열 명 · 영점 50% 에서 8/10 은 **아무 신호가 없어도 우연만으로 절반 넘게**
 * 나온다. 열다섯을 늘어놓고 제일 잘한 것을 고르는 일이 바로 그 우연을
 * 고르는 일이다. 그래서 **열다섯을 다 재도 담당은 정해지지 않았다.**
 *
 * 남는 것은 **미리 정해 놓고 한 번만 잰 가설**뿐이다. 자미 관록궁이
 * 그렇다(전통이 직업을 보라고 지정한 자리 · Fisher p=0.061). 그것도
 * 유의하지 않다. 그러니 이 표의 담당은 **"제일 나은 후보"이지 "맞는 체계"가
 * 아니다.**
 *
 * ── 담당을 둘 수 **없었던** 축 (재고 나서 비웠다) ──────────────
 *   거주 형태     자미 전택궁     4명 중 2   영점 71%
 *   혼인 안정     자미 부처궁     5명 중 3   영점 75%
 *   직업 전환     자미 살파랑     6명 중 1   영점 50%
 *                사주 십성       7명 중 3   영점 50%
 *   학업 경로     자미 문창문곡   5명 중 2   영점 57%  (사주 인성은 통과했다)
 *
 * ── 표본이 모자라 **아직 못 잰** 축 ─────────────────────────
 *   이혼 경험     11명 중 2명뿐 → 영점이 82%라 넘을 수가 없다
 *   동거 여부     해당 4~5명 · 전통 배당을 못 찾았다
 *   건강 부위 2명 · 재정난 2명 · 부업 1명 · 연상연하 1명
 *
 * ── **축이 성립하지 않는** 것 ───────────────────────────────
 *   합격·불합격 같은 판정 — 바깥 상대(회사·심사위원)의 결정이라 명반에
 *   들어갈 자리가 없다. 후보 목록에 '안 된 쪽'이 아예 없는 구조 문제도 겹친다.
 *   시기 전반 — 달 p=0.868 · 해 p=0.196. 서로 다른 두 자료에서 다 졌다.
 *
 * ── 넣지 않은 것 ───────────────────────────────────────────
...
const AXIS_OWNER = {
  ...
  /**
   * 직업 전환 — 한 우물을 파는가, 판을 여러 번 바꾸는가.
   *
   * **이 축은 영점이 50%다.** 열 명이 정확히 다섯씩 갈려서, 지금까지 잰
   * 어느 축보다 재기 좋은 자리였다. 그런데 둘 다 떨어졌다.
   *
   *   자미 관록궁 동성/정성(살파랑)   말한 6명 중 1
   *   사주 비겁식상 vs 관성인성        말한 7명 중 3
   *
   * 자미 쪽이 6분의 1이라 **뒤집으면 6분의 5**가 된다. 뒤집지 않는다.
   * 여섯 명에서 한 번 뒤집어 맞는 것은 자료를 보고 규칙을 고르는 일이고,
   * 그렇게 만든 규칙은 사람이 바뀌면 사라진다. 살파랑은 두수가 변동 격국에
   * 붙여 둔 이름이라 방향이 정해져 있고, 그 방향으로 졌다.
   */
  직업전환: {
    owners: [], grade: '비움',
    note: '열다섯을 다 재도 순열검정 p=0.423 — 1위(토정 8/10)가 우연과 구별되지 않는다',
    say: '이직을 할 사람인지 한 우물을 팔 사람인지 **말하지 않는다**',
  },
  직업: {
