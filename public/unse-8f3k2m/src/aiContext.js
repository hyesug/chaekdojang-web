...
export function buildContext(form, r, f = null) {
  ...
  if (f) {
    ...
    // 수술 후보는 일반 일진과 별도로 근거를 모으되, 절대 순위로 고정하지 않는다.
    // 천의·황도·원국/대운 관계·건강 흐름을 내부 참고점수로 합쳐 후보를 정리한다.
    const surgery = rankSurgeryDays(r.input, r.chart, days);
    const topSurgery = surgery.candidates.slice(0, 10);
    out.push('## 수술·시술 택일 참고 후보');
    out.push('아래는 사이트 내부 규칙으로 계산한 참고 후보다. 확정 택일이나 통서 전체의 판정으로 표현하지 말 것.');
    out.push('내부 점수는 일지충·양인 제외 후 천의·황도 가점, 원국/대운 충·형·해·파 감점, 절기월·당일 건강 흐름과 일반 일진을 함께 반영한다.');
    out.push('천의는 일부 택일법의 치료 관련 참고 신호일 뿐 求醫/治病 宜忌·十二直·통서 전체와 동일하지 않다. 외부 자료나 사용자가 제시한 다른 산법이 있으면 내부 후보와 분리해 비교하고, 내부 순위를 절대값처럼 방어하지 말 것.');
    for (const [i, x] of topSurgery.entries()) {
      ...
      out.push(
        ...
        `참고점수 ${s.referenceScore} · 충감점 ${s.clashPenalty} · 형해파감점 ${s.minorPenalty} · ` +
  ...
  out.push('택일의 기준: 몸에 손대는 일(수술·시술·치료 시작)은 위의 "수술·시술 택일 참고 후보"를 1차 자료로 쓴다. 일지충·양인은 강하게 제외하고, 천의·황도·원국/대운 충형파해·건강 흐름·일반 일진을 함께 본다. 천의나 황도 하나만으로 확정하지 말고, 외부 통서·十二直·求醫/治病 자료가 주어지면 서로 다른 기준으로 구분해 설명한다. 내부 참고순위를 "확정", "재정렬 불가", "이 기준이 정답"처럼 표현하지 말 것. 계약·문서·면접은 황도와 일반 등급을 함께 보고, 이사·출발은 역마, 부탁·지원 요청은 천을을 참고한다.');
