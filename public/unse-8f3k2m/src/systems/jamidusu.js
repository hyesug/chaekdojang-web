...
/** 납음오행 → 국수 */
const GUK = { 4: { n: 2, name: '수이국', hanja: '水二局' }, 0: { n: 3, name: '목삼국', hanja: '木三局' },
...
/** 자미계 — 자미에서 역행으로 붙는 간격 */
const ZIWEI_GROUP = [['자미', 0], ['천기', -1], ['태양', -3], ['무곡', -4], ['천동', -5], ['염정', -8]];
/** 천부계 — 천부에서 순행으로 붙는 간격 */
const TIANFU_GROUP = [['천부', 0], ['태음', 1], ['탐랑', 2], ['거문', 3], ['천상', 4], ['천량', 5], ['칠살', 6], ['파군', 10]];
...
const SIHWA_LABEL = ['화록 (재물과 기회가 붙는다)', '화권 (권한과 주도권이 생긴다)',
...
export function analyze(input) {
  ...
  readings.push({
    ...
    text: spouseStars.length
      ? `${spouseStars.join('·')}이(가) 들었습니다. ${STARS[spouseStars[0]]?.text.split('.')[0]}에 가까운 상대와 인연이 깊습니다. 이 별의 성질이 배우자의 기질이자, 이 사람이 관계에서 반복해서 마주치는 주제가 됩니다.`
