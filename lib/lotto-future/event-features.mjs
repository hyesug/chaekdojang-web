...
export function eventFeatures({ year, month, day, hour, minute }) {
  ...
  const life = digitRoot(String(year).split('').reduce((sum, digit) => sum + Number(digit), 0) + month + day); add('kabbalah.life_path', life, 22); add('kabbalah.birthday_number', digitRoot(day), 9);
