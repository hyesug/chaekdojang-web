...
// 적응형 사고는 생각한 토큰도 출력에 들어간다. 4,000 으로 두면 전체 풀이가
// 문장 중간에서 잘린다. 스트리밍이라 크게 잡아도 타임아웃 걱정이 없다.
...
5. **점수를 그대로 옮기지 마세요.** 아래 자료에는 '총운 71' 같은 숫자가 있지만 이건 열다섯 체계를 평균 낸 내부 값이라 그 자체로는 알려주는 게 없습니다. 화면에도 숫자는 보여주지 않습니다. "총평 71, 학업 72" 처럼 나열하지 말고 "학업 쪽이 가장 낫고 애정이 그다음" 같은 말로 바꿔 쓰세요.

6. **겁주지 마세요.** 나쁜 배치도 사실대로 전하되, 무엇을 조심하면 되는지 실용적으로 씁니다. 건강·재물에 대해 공포를 조장하지 않습니다.
